...
class ChatRoomViewController: UIViewController, UITableViewDelegate, UITextFieldDelegate, UITableViewDataSource {
    ...
    @IBAction func tapButtom(_ sender: Any) {
        ...
        self.pice.sendMessage(message: textField.text as! String)
        textField.text = ""
        try! context.save()
    }
    ...
    override func viewDidLoad() {
        ...
        */
        ...
        queue.async {
            self.pice.senderServer()
            self.pice.networkStuff()
            self.pice.listenServer()
        }
    ...
    func getData(){
        ...
        do{
            ...
            for i in 0...(result as! [NSManagedObject]).count - 1{
                self.counter += 1
                print("i: ",i, (result as! [NSManagedObject])[i].value(forKey: "text"))
